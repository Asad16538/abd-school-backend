...
# License Variables
VERIFICATION_STORE = {}
LICENCE_DAYS = 30  # Default 30 days
...
def init_db():
    ...
    # =====================================================================
    # 🔑 YEH NAYA TABLE ADD KARO (LICENCES)
    # =====================================================================
    
    # 24. Licences Table (KEY STORAGE)
    execute_query(cursor, '''
        CREATE TABLE IF NOT EXISTS licences (
            id SERIAL PRIMARY KEY,
            licence_key TEXT UNIQUE NOT NULL,
    ...
    # 25. Licence Devices Table (DEVICE BINDING)
    execute_query(cursor, '''
...
# =====================================================================
# 🔑 NEW LICENSE SYSTEM WITH EXPIRY, DOMAIN & DEVICE BINDING
# =====================================================================

@app.route('/api/licence/generate', methods=['POST'])
def generate_licence():
    """🔑 Generate new license with expiry"""
    data = request.json
    client_name = data.get('client_name', 'Unknown')
    client_email = data.get('client_email', '')
    client_mobile = data.get('client_mobile', '')
    client_domain = data.get('domain', '').strip()
    ...
    licence_key = f"ABD-{uuid.uuid4().hex[:8].upper()}"
    ...
    try:
        execute_query(cursor, '''
            INSERT INTO licences (licence_key, client_name, client_email, client_mobile, client_domain, start_date, expiry_date, device_limit, is_active)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, 1)
        ''', (licence_key, client_name, client_email, client_mobile, client_domain, start_date, expiry_date, device_limit))
        
        conn.commit()
        conn.close()
        
        # 📱 Telegram Notification
        msg = f"""🔑 *NEW LICENSE GENERATED*
        
🆔 *Key:* `{licence_key}`
📛 *Client:* {client_name}
📧 *Email:* {client_email}
📱 *Mobile:* {client_mobile}
🌐 *Domain:* {client_domain or 'Not Set'}
📅 *Start:* {start_date}
📅 *Expiry:* {expiry_date}
⏳ *Days:* {days}
📱 *Device Limit:* {device_limit}

_Generated by A.B.Digital Work_"""
        ...
        return jsonify({
            ...
            "message": f"✅ License generated! Valid for {days} days."
        })
...
def verify_licence_new():
    """✅ Verify license with expiry, domain & device check"""
    data = request.json
    licence_key = data.get('key', '').strip().upper()
    ...
    # 🔍 Check if key exists
    execute_query(cursor, '''
        SELECT id, licence_key, client_name, client_domain, start_date, expiry_date, is_active, device_limit
        FROM licences 
        WHERE licence_key = ?
    ''', (licence_key,))
    ...
    licence_id, key, client_name, client_domain, start_date, expiry_date, is_active, device_limit = row
    
    # 🔴 Check if active
    if is_active != 1:
        conn.close()
        return jsonify({"success": False, "error": "❌ License is deactivated!"}), 403
    
    # 🔴 Check expiry
    today = datetime.now().strftime("%Y-%m-%d")
    if expiry_date < today:
        conn.close()
        return jsonify({"success": False, "error": f"❌ License expired on {expiry_date}! Please renew."}), 403
    
    # 🔴 DOMAIN CHECK (if domain is set in license)
    if client_domain and client_domain != domain:
        conn.close()
        return jsonify({
            "success": False, 
            "error": f"❌ This key is locked to domain: {client_domain}"
        }), 403
    
    # 🛡️ DEVICE BINDING CHECK
    if device_id:
        # Check if device already registered
        execute_query(cursor, '''
            SELECT id FROM licence_devices 
            WHERE licence_id = ? AND device_id = ?
        ...
        if not device_exists:
            ...
            execute_query(cursor, '''
                SELECT COUNT(*) FROM licence_devices 
                WHERE licence_id = ? AND is_active = 1
            ''', (licence_id,))
            ...
            if device_count >= device_limit:
                ...
                return jsonify({
                    "success": False,
                    "error": f"❌ Device limit reached! Max {device_limit} devices allowed."
                }), 403
            
            # Register new device
            now_str = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
            execute_query(cursor, '''
                INSERT INTO licence_devices (licence_id, device_id, first_seen, last_seen)
                VALUES (?, ?, ?, ?)
            ''', (licence_id, device_id, now_str, now_str))
            conn.commit()
        
        # Update last seen
        execute_query(cursor, '''
            UPDATE licence_devices SET last_seen = ? 
            WHERE licence_id = ? AND device_id = ?
        ''', (datetime.now().strftime("%Y-%m-%d %H:%M:%S"), licence_id, device_id))
        conn.commit()
    
    # Update last accessed
    execute_query(cursor, '''
        UPDATE licences SET last_accessed = ? WHERE id = ?
    ''', (datetime.now().strftime("%Y-%m-%d %H:%M:%S"), licence_id))
    conn.commit()
    ...
    return jsonify({
        ...
        "device_limit": device_limit,
        "is_active": True,
        "message": f"✅ Valid License. {days_left} days remaining."
    })


@app.route('/api/licence/renew', methods=['POST'])
def renew_licence():
    """🔄 Renew license (extend days)"""
    data = request.json
    licence_key = data.get('key', '').strip().upper()
    extra_days = int(data.get('extra_days', 30))
    ...
    # Check if key exists
    execute_query(cursor, '''
        SELECT id, expiry_date, client_name FROM licences WHERE licence_key = ?
    ''', (licence_key,))
    ...
    licence_id, current_expiry, client_name = row
    
    # Calculate new expiry (extend from today)
    new_expiry = (datetime.now() + timedelta(days=extra_days)).strftime("%Y-%m-%d")
    
    # Update
    execute_query(cursor, '''
        UPDATE licences SET expiry_date = ?, is_active = 1 WHERE id = ?
    ''', (new_expiry, licence_id))
    
    conn.commit()
    conn.close()
    
    # 📱 Telegram Notification
    msg = f"""🔄 *LICENSE RENEWED*
...
@app.route('/api/licence/status/<licence_key>', methods=['GET'])
def get_licence_status(licence_key):
    """📊 Get license status"""
    from datetime import datetime
    ...
    execute_query(cursor, '''
        SELECT licence_key, client_name, client_email, client_mobile, client_domain,
               start_date, expiry_date, is_active, device_limit, created_at
        FROM licences 
        WHERE licence_key = ?
    ''', (licence_key.upper(),))
    ...
    expiry_dt = datetime.strptime(row[6], "%Y-%m-%d")
    ...
    return jsonify({
        ...
        "client_email": row[2],
        "client_mobile": row[3],
        "client_domain": row[4] or 'Not Set',
        "start_date": row[5],
        "expiry_date": row[6],
        "is_active": row[7] == 1,
        "device_limit": row[8],
        "created_at": row[9],
        "days_left": days_left,
        "status": "Active" if row[7] == 1 and days_left > 0 else "Expired/Inactive"
    })


@app.route('/api/licence/all', methods=['GET'])
def get_all_licences():
    ...
    execute_query(cursor, '''
        SELECT licence_key, client_name, client_domain, start_date, expiry_date, is_active 
        FROM licences 
        ORDER BY id DESC
        LIMIT 100
    ''')
    ...
    for r in rows:
        licences.append({
            "key": r[0],
            "client": r[1],
            "domain": r[2] or 'Not Set',
            "start": r[3],
            "expiry": r[4],
            "active": r[5] == 1
        })
    
    return jsonify({"success": True, "licences": licences})


@app.route('/api/licence/expired', methods=['GET'])
def get_expired_licences():
    """⚠️ Get all expired licenses"""
    from datetime import datetime
    today = datetime.now().strftime("%Y-%m-%d")
    ...
    execute_query(cursor, '''
        SELECT licence_key, client_name, client_email, expiry_date 
        FROM licences 
